...
namespace AlgorithmsPractice.Codility.Lesson7_StacksAndQueues
{
    ...
    then it meets fish number 3 and eats it too. Finally, it meets fish number 4 and is eaten by it. The remaining two fish, number 0 
    ...
    {
        public static int GetRemainingFishs(int[] sizes, int[] directions)
        {
            var lenght = sizes.Length;
            var survival = 0;

            var stack = new Stack<Fish>();
            ...
            for (int i = 0; i < lenght - 1; i++)
            {
                ...
                if (direction == 0)
                {
                    ...
                    if (survives) survival++;
                }
                else
                {
                    stack.Push(new Fish(size, direction));
                }
            }

            return survival + stack.Count;
        }
    }
